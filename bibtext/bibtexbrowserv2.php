...
// security fix.
// @include('bibtexbrowser.local.php',__FILE__));
...
@define('TYPES_SIZE',5); // number of entry types per table
@define('YEAR_SIZE',10); // number of years per table
@define('AUTHORS_SIZE',10); // number of authors per table
@define('TAGS_SIZE',10); // number of keywords per table
...
// Ensure $delegate is properly initialized
$delegate = new YourDelegateClass();
$parser = new StateBasedBibtexParser($delegate);
$parser->parse('sog.bib');
    ...
    function parse($bibfilename) {
        ...
        if ($delegate === null) {
            throw new Exception('Error: $delegate is not initialized.');
        }
  ...
  */
  ...
  function bib2links() {
    ...
    if ($this->hasField('title')) {
      $str .=' Look for this document on <a target="_blank" href="https://scholar.google.ca/scholar?q='.$this->getField("title").'">Google Scholar</a>' ;
    }

    if ($this->hasField('title')) {
    $str .=' or <a target="_blank" href="https://google.ca/#q='.$this->getField("title").'">Google</a>' ;
  }

    if ($this->hasField('doi')) {
      $str .= ' or <a target="_blank" href="http://dx.doi.org/'.$this->getField("doi").'"> locate using its doi</a>';
    }
...
class MenuManager {
  ...
  function searchView() {
    ?>
    <form action="?" id="bibsearch" method="get" target="<?php echo BIBTEXBROWSER_MENU_TARGET;?>">
      <input type="text" class="input_box" id="search_text" size="18"/>
      <input type="hidden" name="<?php echo Q_SEARCH; ?>" id="search_value" />
      <input type="hidden" name="<?php echo Q_FILE; ?>" value="<?php echo $_GET[Q_FILE]; ?>"/>
      <br/>
      <select style="width:95%;" id="search_type">
        <option value="">- Search Type -</option>
        <option value="and">And</option>
        <option value="or">Or</option>
        <option value="string">String</option>
      </select>
      <input type="submit" value="Search" class="input_box" id="submit" /></div>
    ...
    $('#submit').click(function() {
        ...
        $('#search_text').each(function(){
            ...
            if($('#search_type').val() == 'and') {
              //iterate through each of the "words" and add "add" regex formatting
              for (var i = 0, len = split.length; i < len; i++) {
                  split[i] = '(?=.*' + split[i].slice(0) + ')';
              }
              //re-join the string and set the value of the element
              this.value = split.join('');
            }

            if($('#search_type').val() == 'or') {
              //iterate through each of the "words" and add "or" character
              for (var i = 0, len = split.length; i < len; i++) {
                  split[i] = split[i].slice(0) + '|';
              }
              //re-join the string and set the value of the element
...
class Dispatcher {
  ...
  function frameset() {    ?>
    ...
    <frameset cols="19%,*">
    <frame name="menu" id="menu" src="<?php echo '?'.Q_FILE.'='. urlencode($_GET[Q_FILE]).'&amp;menu'; ?>" />
    <frame name="main" id="main" src="<?php echo '?'.Q_FILE.'='. urlencode($_GET[Q_FILE]).'&amp;year=latest'?>" />
    </frameset>
